...
public:
   ...
   void grad(const Vector &x, Vector &grad) const override
   {
      const int siz = x.Size();
      const real_t *xp = x.Read();
      const real_t *xminp = xmin.Read();
      const real_t *xmaxp = xmax.Read();
      real_t *invgp = grad.Write();
      MFEM_FORALL(i, siz,
      {
         real_t diff = xmaxp[i] - xminp[i];
         real_t val = diff > tol ? (xp[i] - xminp[i]) / diff
         : 0.5;
         invgp[i] = FermiDirac::grad(val);
      });
   }
   ...
   void gradinv(const Vector &x, Vector &invg) const override
   {
      ...
      const real_t *xminp = xmin.Read();
      const real_t *xmaxp = xmax.Read();
      real_t *invgp = invg.Write();
      MFEM_FORALL(i, siz,
      {
         real_t val = FermiDirac::gradinv(xp[i]);
         invgp[i] = xminp[i] + (xmaxp[i] - xminp[i]) * val;
      });
   }
